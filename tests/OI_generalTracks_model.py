...
def build_dataset(arr, file_labels_in, useStandaloneFeatures=True, verbose=False):
    ...
    if useStandaloneFeatures:
        ...
        qualityScoreMatrix = chi2EtaMatrix/maxChi2 + chi2PhiMatrix/maxChi2 + chi2PtMatrix/maxChi2 + chi2DzMatrix/maxChi2
...
def main():
    ...
    if global_rank == 0:
        ...
        plt.grid(True)
        plt.savefig("OIGeneralOutput/roc_curve.png")
        plt.close()
        ...
        plt.grid(True)
        plt.savefig("OIGeneralOutput/metrics_curve.png")
        plt.close()
    ...
    if global_rank == 0:
        ...
        # Save threshold info
        with open("OIGeneralOutput/thresholds.txt", "w") as f:
