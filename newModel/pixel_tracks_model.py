...
def main():
    ...
    if "muon_pixel_tracks_pt" in feature_names:
        pt_idx = feature_names.index("muon_pixel_tracks_pt")
        ...
        print("Warning: 'muon_pixel_tracks_pt' not found. Using uniform weights.")
    ...
    if global_rank == 0:
        ...
        plt.grid(True)
        plt.savefig("pixelTracksOutput/roc_curve.png")
        plt.close()
        ...
        plt.grid(True)
        plt.savefig("pixelTracksOutput/metrics_curve.png")
        plt.close()
    ...
    if global_rank == 0:
        ...
        torch.onnx.export(
            model_for_export,
            dummy_input,
            "pixelTracksOutput/pixel_track_fused.onnx",
            export_params=True,
            opset_version=13,
            input_names=["input"],
            output_names=["output"],
        ...
        print("Saved: pixel_track_fused.onnx (expects RAW unscaled inputs)")

        # Save threshold info
        with open("pixelTracksOutput/thresholds.txt", "w") as f:
